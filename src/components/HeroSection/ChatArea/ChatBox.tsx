...
const EmojiPicker = dynamic(() => import('emoji-picker-react'), {
  ssr: false,
  loading: () => <div className="flex items-center justify-center p-4"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-500"></div></div>,
});

const ChatBox = () => {
    const convParti = useStore(store => store.convParti)
    const setConvParti = useStore(store => store.setConvParti)
    ...
    // Preload EmojiPicker component when component mounts
    useEffect(() => {
        const preloadEmojiPicker = async () => {
            try {
                await import('emoji-picker-react');
            } catch (error) {
                console.error('Failed to preload EmojiPicker:', error);
            }
        };
        ...
        try {
            ...
            if (response) {
                if(messageIds?.length==0){
                    setConvParti({...convParti, conversationId:response.conversationId})
                }
        ...
        <div className='w-full absolute bottom-3'>
            ...
            <div className='pl-3 flex w-full'>
                <div className='w-24 bg-white rounded-l-full flex justify-between px-[6px] items-center '>
                    ...
                    <span 
                        ref={emojiButtonRef}
                        className='h-10 w-10 hover:bg-gray-50 rounded-full flex items-center justify-center cursor-pointer'
                        onClick={(e) => {
                            e.stopPropagation();
                ...
                <div className=' w-full flex mr-2'>
                    <input className=' w-full h-12 focus-visible:ring-0 border-0 outline-0 active:border-0 active:outline-0 emoji-font' placeholder='Type a message' value={msg} onChange={(e) => setMsg(e.target.value)} onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                            sendMsg();
                        }
                    }} />
